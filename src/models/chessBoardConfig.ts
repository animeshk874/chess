...
export class ChessBoardConfig{
  ...
  constructor(skipPiecesConfig: boolean = false){
  ...
  assignPieces(){
    this.boardMatrix[0].forEach((square: BlockData, cellIndex: number) => {
      assignPiecesForColor.call(this, Colors.BLACK, 0, cellIndex);
    });
    this.boardMatrix[7].forEach((square: BlockData, cellIndex: number) => {
    ...
    function assignPiecesForColor(colorType: Colors, rowIndex: number, cellIndex: number){
